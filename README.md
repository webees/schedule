...
- [亮点](#-亮点) · [架构](#-架构) · [原子锁](#-原子锁) · [自愈](#-自愈) · [容错](#-容错)
- [文件](#-文件) · [核心函数](#-核心函数) · [扩展](#-扩展) · [测试](#-测试) · [启动](#-启动)
...
| `dispatch(round_num, time_str, ...)` | 竞锁 + 触发 + 日志 |
| `trigger(repo, wf)` | 使用 PAT 跨仓库触发 workflow |
| `is_expired(lock_tag, now_epoch, now_minute)` | 锁过期判断 (cron/秒级/旧格式兼容) |
